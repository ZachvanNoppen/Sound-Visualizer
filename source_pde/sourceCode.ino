...
void loop(){
  ...
  if(anPos == 0){ 
    ...
    animationFive(threshold,cProfile,startIndex);
  }
}
/* TEST ROUTINES
 *  
...
void animationFour(int threshold, String cProfile[2], uint8_t colorIndex){
    //Making a custom pallete based on input
    CRGB c1 = hexToRGB(cProfile[4]);
    CRGB c2 = hexToRGB(cProfile[5]);
    TProgmemPalette16 custColors_p PROGMEM =
      {
          c1,
          c1,
     ...
     CRGBPalette16 currentPalette = custColors_p; 
     for( int i = 0; i < MAX_LEDS; i++) {
...
void animationFive(int threshold, String cProfile[6], uint8_t colorIndex){
